...
export const GetColorTheme = (theme: Theme, color?: ColorScheme, def?: string) => {
  ...
  const color_found = (color_find ?? def ?? theme.colors.foreground[100]) as string
  return color_found
}
...
export const GetFontFamilyTheme = (theme: Theme, family?: keyof typeof theme.fontFamily, def?: string) => {
  if (family === undefined) return def
  const family_get = get(theme.fontFamily as any, family)
  const family_found = (family_get ?? def ?? theme.fontFamily.regular) as string
  return family_found
}
